...
#include <Wire.h>
#include <Adafruit_Sensor.h>    // Requerido por la versión Unified
#include <Adafruit_BMP085_U.h>  // Librería Unified para el sensor

// --- INSTANCIA DEL SENSOR BMP180 (Versión Unified) ---
// El número 10085 es un ID arbitrario para el sensor
...
int humorAmbiente = 0;
float temperaturaActual = 0.0; // Se lee del BMP180
int nivelAgua = 0;              
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpCode > 0) {
      ...
      if (!error) {
        ...
        Serial.printf("Clima actualizado: %s, Humedad ext: %d%%\n", weatherMain, humorAmbiente);
      }
    }
    http.end();
  }
...
void setup() {
  ...
  // Iniciar bus I2C con los pines definidos
  Wire.begin(BMP_SDA, BMP_SCL);

  // Inicializar sensor BMP180 Unified
  if (!bmp.begin()) {
    Serial.println("No se encontró el sensor BMP180, revisa las conexiones!");
  } else {
...
void loop() {
  ...
  if (millis() - lastUpdate > 2000) {
    ...
    // Leemos la temperatura localmente desde el BMP180 (Versión Unified)
