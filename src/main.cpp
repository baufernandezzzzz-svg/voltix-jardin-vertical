...
<head>
  ...
  <style>
    ...
    /* Contenedor SVG para Gauge Circular (Estilo Foto) */
    ...
    /* Valores numéricos dentro del círculo */
    ...
    /* Botones de control estilo píldora */
...
<body>
  ...
  <div class="content">
    ...
    <button class="btn-pill" id="btn_mode" onclick="toggleMode()">Automatico</button>
  </div>
...
<script>
  ...
  // Controla la longitud del arco del círculo (Max de stroke-dashoffset es 330, cubre 240 grados aprox)
  function updateGauge(gaugeId, textId, val) {
    if(val > 100) val = 100;
    if(val < 0) val = 0;
    var circle = document.getElementById(gaugeId);
    // Mapeo: a mayor valor, menor offset (se dibuja la línea)
  ...
  function onMessage(event) {
    ...
    if(data.pump) {
      ...
      document.getElementById('status_led').style.backgroundColor = "#2ecc71"; // Verde si bombea
    } else {
      pumpBtn.innerText = "Pump Off";
      pumpBtn.style.background = "transparent";
      pumpBtn.style.color = "#8e44ad";
      document.getElementById('status_led').style.backgroundColor = "#d64756"; // Rojo en reposo
    }
  }
